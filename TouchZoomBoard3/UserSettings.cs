...
namespace TouchZoomBoard
{
    internal sealed class UserSettings
    {
        ...
        internal double HighlighterWidth { get; set; } = HighlighterWidthPolicy.Default;
        ...
        internal static UserSettings Load()
        {
            ...
            try
            {
                using (var key = Registry.CurrentUser.OpenSubKey(SettingsPath))
                {
                    ...
                    if (double.TryParse(
                        Convert.ToString(key.GetValue("HighlighterWidth", null)),
                        NumberStyles.Float,
                        CultureInfo.InvariantCulture,
                        out highlighterWidth))
                    {
                        int widthVersion;
                        int.TryParse(Convert.ToString(key.GetValue("HighlighterWidthVersion", 0)),
                            NumberStyles.Integer, CultureInfo.InvariantCulture, out widthVersion);
                        settings.HighlighterWidth = HighlighterWidthPolicy.Restore(highlighterWidth, widthVersion);
                    }
        ...
        internal void Save()
        {
            try
            {
                using (var key = Registry.CurrentUser.CreateSubKey(SettingsPath))
                {
                    ...
                    key.SetValue("HighlighterWidthVersion", HighlighterWidthPolicy.SettingsVersion, RegistryValueKind.DWord);

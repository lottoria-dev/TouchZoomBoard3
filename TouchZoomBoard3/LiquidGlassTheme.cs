...
namespace TouchZoomBoard
{
    ...
    internal static class LiquidGlassTheme
    {
        ...
        internal static bool ApplyWindow(Window window, HwndSource source, IntPtr hwnd, string surface)
        {
            ...
            var perPixelWindow = window.AllowsTransparency;
            try
            {
                ...
                // 안전 배경으로 대체한다.
                if (!accepted && !clearPanel && !perPixelWindow)
                {
            ...
            {
                window.Background = clearPanel || perPixelWindow
                    ? Brushes.Transparent
                    : new SolidColorBrush(FallbackWindowColor);
                try
                {
                    if (source.CompositionTarget != null)
                        source.CompositionTarget.BackgroundColor = clearPanel || perPixelWindow
                            ? Colors.Transparent
                            : FallbackWindowColor;
                }
                catch
        ...
        internal static bool ApplyPopup(HwndSource source, string surface)
        {
            ...
            try
            {
                // WPF ToolTip/ContextMenu and our option/ComboBox popups use
                // layered HWNDs. Their transparent corners must stay transparent;
                // a rectangular DWM backdrop or opaque fallback fills them again.
                if (source.CompositionTarget != null)
                    source.CompositionTarget.BackgroundColor = Colors.Transparent;
                NativeMethods.ClearWindowRegion(source.Handle);
                WriteResult(surface, source.Handle, true, 0, "perPixelPopup=True, blur=False", 0);
                return true;
            }
            catch (Exception exception)
            {

    ...
    internal sealed class AppController : IDisposable
    {
        ...
        internal AppController(Application application, bool startedByTouchRefresh)
        {
            ...
            using (var process = Process.GetCurrentProcess())
                externalFocus = new ExternalFocusState((uint)process.Id);
        ...
        {
            var createdPanel = new ControlPanelWindow(settings.TooltipsEnabled, externalFocus);
            panelGeneration++;
        ...
        private void EnsureOverlay()
        {
            ...
            // Resolve the current panel at input time (the panel can be recreated
            // after a display/touch-device change). Do not consume the first stroke.
            overlay.InputStarted += () => panel?.DismissToolPopupsForInput();
            overlay.InputFinished += () => panel?.RestorePresentationFocus("overlay-input-finished");
        ...
        {
            settings.HighlighterWidth = HighlighterWidthPolicy.Normalize(width);
        ...
        private static bool IsShapeMode(AppMode value)
        {
            return value == AppMode.Rectangle || value == AppMode.Ellipse ||
                   value == AppMode.Circle || value == AppMode.Line || value == AppMode.Arrow;
        }
        ...
        private bool RestoreExternalForegroundWindow(IntPtr target, uint expectedProcessId, string reason)
        {
            ...
            if (targetProcessId == 0 || targetProcessId == currentProcessId ||
            ...
            NativeMethods.GetWindowThreadProcessId(before, out foregroundProcessId);
            var inputWindows = CaptureSessionWindowHandles()
                .Concat(new[] { panel.WindowHandle }).Concat(panel.GetPopupWindowHandles());
            // A mode/zoom change can leave our overlay or minimap active. Return
            // focus from those known input windows too, but never from another app
            // or a text-entry/settings window belonging to this process.
            if (!ExternalFocusState.CanRestoreFrom(before, foregroundProcessId, currentProcessId,
                settingsWindow?.IsVisible == true, inputWindows)) return false;
            var requested = NativeMethods.SetForegroundWindow(target);
